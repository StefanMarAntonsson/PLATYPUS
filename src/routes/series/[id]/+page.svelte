<script lang="ts">
  ...
  import {
    appData, updateSeries, deleteSeries,
  ...
  const series = $derived(appData.series.find(s => s.id === seriesId));
  const mediaList = $derived(series ? getSeriesMedia(seriesId) : []);
  ...
  $effect(() => {
    if (series) {
      nameInput = series.name;
      originalInput = series.originalName ?? '';
    }
  });

  function startEdit() { editing = true; }
  function cancelEdit() { editing = false; if (series) { nameInput = series.name; originalInput = series.originalName ?? ''; } }

  function saveEdit() {
    if (!nameInput.trim()) return;
    updateSeries(seriesId, { name: nameInput.trim(), originalName: originalInput.trim() || null });
    editing = false;
  }
...
{#if contextMenu}
  <div role="presentation" class="fixed inset-0 z-50" onclick={() => contextMenu = null} onkeydown={e => e.key === 'Escape' && (contextMenu = null)}>
    <div
    ...
    >
      ...
      <button
        ...
        onclick={() => { removeMediaFromSeries(seriesId, contextMenu!.mediaId); contextMenu = null; }}
...
<ConfirmDialog
  open={confirmDelete}
  title="Delete series"
  message="Delete &quot;{series?.name}&quot;? Anime in your library will not be affected."
...
<div class="p-4 md:p-6 space-y-6">
  <!-- Back -->
  <a href="{base}/series" class="text-sm text-zinc-500 hover:text-zinc-300 transition-colors flex items-center gap-1">
    ← Series
  </a>

  {#if !series}
    <p class="text-zinc-500">Series not found.</p>
  {:else}
    <!-- Title -->
      ...
      <div class="flex-1 min-w-0">
        ...
        {:else}
          <div class="flex items-start gap-3">
            <div class="flex-1">
              <h1 class="text-2xl font-bold text-white">{series.name}</h1>
              {#if series.originalName}
                <p class="text-sm text-zinc-400 mt-0.5">{series.originalName}</p>
              {/if}
            </div>
    ...
    <div class="border border-border rounded-xl p-4 space-y-3">
      <h3 class="text-sm font-semibold text-zinc-300">Titles in this series</h3>
          ...
          {#each mediaList as m (m.id)}
            <div class="flex items-center gap-2 p-2 rounded-lg bg-zinc-800/50">
              ...
              <button
                ...
                onclick={() => removeMediaFromSeries(seriesId, m.id)}
              >×</button>
            </div>
          {/each}
        </div>
    ...
    {:else}
      <div class="text-center py-10 text-zinc-500 text-sm">
        Add anime to this series to track episodes.
      </div>
    {/if}
  {/if}
</div>
